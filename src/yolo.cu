...
enum class NormType : int { None = 0, MeanStd = 1, AlphaBeta = 2 }; //定义一个枚举类，包含三个枚举值
...
inline int upbound(int n, int align = 32) { return (n + align - 1) / align * align; } //定义一个内联函数，用于将整数n向上对其到32的倍数
...
static __global__ void decode_kernel_common(float *predict, int num_bboxes, int num_classes,
  ...
  float *pitem = predict + output_cdim * position; //得到当前线程需要处理的边界框的预测结果所在的位置
  ...
  float *pout_item = parray + 1 + index * NUM_BOX_ELEMENT; //parry的第一个元素存储的是当前检测到的有效的边界框数量，所以在存储具体的结果时还需要
                                                                //加上偏移量1
  *pout_item++ = left;
  *pout_item++ = top;
  *pout_item++ = right;
...
static __global__ void decode_kernel_v8(float *predict, int num_bboxes, int num_classes,  //v8相较于通用的解码区别在于类别置信度、 总的置信度
                                        int output_cdim, float confidence_threshold,
                                        float *invert_affine_matrix, float *parray,
                                        int MAX_IMAGE_BOXES) {
  int position = blockDim.x * blockIdx.x + threadIdx.x;
...
static __global__ void fast_nms_kernel(float *bboxes, int MAX_IMAGE_BOXES, float threshold) {
  ...
  float *pcurrent = bboxes + 1 + position * NUM_BOX_ELEMENT;  //bboxes就是上述的parry
...
static __global__ void warp_affine_bilinear_and_normalize_plane_kernel(
    ...
    int dst_height, uint8_t const_value_st, float *warp_affine_matrix_2_3, Norm norm) { //用于对输入图像进行仿射变换、双线性插值和归一化处理
  int dx = blockDim.x * blockIdx.x + threadIdx.x;
  int dy = blockDim.y * blockIdx.y + threadIdx.y;  //线程索引计算
